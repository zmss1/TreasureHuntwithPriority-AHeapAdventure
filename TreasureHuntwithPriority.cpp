...
#include <algorithm> // For algorithms (e.g., sort, swap)

using namespace std;

// Function to display the heap
void displayHeap(const vector<int>& heap) {
    cout << "💎 Current Treasure Priorities (Heap): ";
...
void removeSpecificTreasure(vector<int>& heap, int priority, bool isMaxHeap) {
    ...
    cout << "🗑️ Removing treasure with priority " << priority << "." << endl;
...
void treasureHuntWithPriority() {
    ...
    while (true) {
        ...
        if (choice == 'A' || choice == 'a') {
            ...
            if (heap.empty()) {
                cout << "⚠️ No treasures to remove!" << endl;
                continue;
            }
            int priority;
            ...
            if (heap.empty()) {
                cout << "⚠️ Heap is empty! Cannot convert." << endl;
                continue;
            }
